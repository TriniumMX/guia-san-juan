...
- [x] CSP **en enforcement** (2026-07-17, `middleware.js`): estrategia híbrida — `/admin` con **nonce** estricto (sin `unsafe-inline`; Next nonce sus scripts inline, admin forzado a dinámico), público **enforced** con `unsafe-inline` (obligado por los scripts RSC de Next) pero conservando SSG. Script de tema externalizado (`/public/theme-init.js`). Directivas endurecidas: `object-src 'none'`, `upgrade-insecure-requests`, sin googleapis/gstatic (fuentes autohospedadas con `next/font`). *Nota: quitar `unsafe-inline` del público requeriría render dinámico global — decisión de producto (ver TRI-223).*
